...
class TrafficLightGridEnv(Env):
    ...
    * discrete: determines whether the action space is meant to be discrete or
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        self.traffic_phases = ["GGggrrrGGgrrr", "yyyyrrryyyrrr", "rrrrGGgrrrGGg", "rrrryyyrrryyy"]
        ...
        if self.tl_type != "actuated":
            for i in range(self.rows * self.cols):
                self.k.traffic_light.set_state(
                    node_id='center' + str(i), state=self.traffic_phases[0])
                ...
                if self.last_change[i] >= self.min_switch_time:
                    if self.direction[i] == 0:
                        self.k.traffic_light.set_state(
                            ...
                            state=self.traffic_phases[0])
                    else:
                        self.k.traffic_light.set_state(
                            node_id='center{}'.format(i),
                            state=self.traffic_phases[2])
                    self.currently_yellow[i] = 0
            else:
                if action:
                    if self.direction[i] == 0:
                        self.k.traffic_light.set_state(
                            node_id='center{}'.format(i),
                            state=self.traffic_phases[1])
                    else:
                        self.k.traffic_light.set_state(
                            node_id='center{}'.format(i),
                            state=self.traffic_phases[3])
